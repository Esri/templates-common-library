...
export interface TelemetrySettings {
    ...
    config: any;//ConfigurationSettingsBase,
    appName: string
}
...
export default class Telemetry extends Accessor {
    ...
    @property()
    state: "ready" | "waitingForConsent" | "loading" | "error" = "waitingForConsent";
    ...
    runInit(){
        if(this?.settings?.config?.googleAnalyticsConsent === true && !this.isConsentGiven){
    ...
    private async _initTelemetry(settings){
    ...
    private _isGoogleEnabled(settings: TelemetrySettings) {
        ...
        const { googleAnalytics, googleAnalyticsConsent, telemetry } = settings?.config;
    ...
    private _isAdobeEnabled(settings: TelemetrySettings) {
        // NOTE: googleAnalyticsConsent is just the general Consent Message for all Analytics providers
        const { adobeLaunchAnalytics, googleAnalyticsConsent, telemetry } = settings?.config;
    ...
    private async _loadAdobeAnalytics(settings: any) {
        ...
        return new Promise<void>(async (resolve, reject) => {
            ...
            if (!enableAdobeLaunch && scriptsExist) {
                ...
                let _isLoaded = function () {
                    if (window['s_gi']) {
                        const adobeTracker = window["s_gi"](adobeLaunchAnalyticsReportSuiteId);
    ...
    _adobeLaunchScriptsExist() {
        const alreadyLoaded = this.adobeIds.every(id => {
            return document.getElementById(id) !== null ? true : false;
        });
        return alreadyLoaded;
    }
...
class AdobeWrapper implements TelemetryInstance  {
    ...
    startWorkflow: (name: string,payload?: WorkflowPayload) => any 
        = (name: string,payload?: WorkflowPayload)=>{
            return this._wrappedTelemInstance.startWorkflow(name, payload);
        };
    stepWorkflow: (name: string,step: string,payload?: WorkflowPayload) => void
        = (name: string, step: string,payload?: WorkflowPayload)=>{
    ...
    formatPayload(
        ...
        const { hostname, pathname } = window && window.location ? window.location : { hostname: null, pathname: null }
